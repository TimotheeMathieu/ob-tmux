...
;; Poly-mode for ob-tmux, extend the ob-tmux initial package with language-dependent behavior
;; make use of a :lang header source-block argument (which for now must be at the end of the header line).
;;
;; Heavily inspired by 'poly-org-mode'
;;
;; usage:
;; it is activated either manually or on a file by file basis.
;; by adding a line of the type
;; # -*- mode: org;  mode:visual-line ; mode:poly-tmux -*-
;; as first line of the org file
;; a block can then be defined by
;; #+begin_src tmux :session sess :lang python
;; import numpy as np
;; #+end_src
;; it should get color highlighting and all the nice things of the python major mode available in it.
;;; Code:
